...
```mermaid
graph TD
    User(["👤 Usuário"]) -->|Usa a aplicação| FE

    subgraph Frontend
        FE["SPA | React + Vite"]
    end

    FE -->|"Requisições HTTP/REST"| API

    subgraph Backend
        API["API Principal - NestJS"]
    end

    API -->|"Lê dados (Prisma ORM)"| DB[("🐘 PostgreSQL 16\nnews | categories\nusers | user_preferences")]

    subgraph Processamento Assíncrono
        Agent["🤖 Agente Curador\n(cron 10 min)\n\n• TemplateGenerator\n• CsvAnalyzer\n• KeywordClassifier"]
        Queue["📨 Message Broker\nBullMQ + Redis"]
        Consumer["⚙️ Serviço Consumidor\n(Worker)"]
        AI["☁️ API de IA\nGemini 1.5 Flash"]
    end

    Agent -->|"Publica Artigo Curado"| Queue
    Queue -->|"Notícia em Fila"| Consumer
    Consumer -->|"Chama para sumarizar"| AI
    Consumer -->|"Salva Artigo Processado\n(Prisma ORM)"| DB
```
